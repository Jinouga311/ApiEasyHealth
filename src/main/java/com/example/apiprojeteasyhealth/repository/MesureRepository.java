...
public interface MesureRepository extends JpaRepository<Mesure, Long> {
    ...
    @Query("SELECT new com.example.apiprojeteasyhealth.dto.MesureForPatientAndPathologie(m.valeur, m.unite, m.periode, c.date) "
            + "FROM Mesure m "
            + "JOIN m.suivi s "
            + "JOIN s.consultation c "
            + "JOIN c.pathologie pa "
            + "WHERE m.patient.adresseMail = :mailPatient AND pa.libelle = :libellePathologie AND c.date BETWEEN :startDate AND :endDate "
    ...
    @Query("SELECT new com.example.apiprojeteasyhealth.dto.MesureForPatient(m.valeur, m.unite, m.periode, c.date, pa.libelle)\n" +
            "FROM Mesure m\n" +
            "JOIN m.suivi s\n" +
            "JOIN s.consultation c\n" +
            "JOIN c.pathologie pa\n" +
            ...
            "ORDER BY m.periode DESC")
    List<MesureForPatient> getMesureFromPatient(@Param("mailPatient") String mailPatient, @Param("startDate")LocalDate starDate, @Param("endDate") LocalDate endDate);
